...
type Cell = { id: string; trafficGb: number | null; months: number; title: string | null; price: number; category: string };

export default function PartnerPage() {
  const { home, loading, reload } = useDashAuth(["partner", "admin"]);
  ...
  return (
    ...
    >
      ...
      <div className="grid">
        ...
        <div className="stat">
          <div className="label">سفارش‌ها</div>
          <div className="value">{report?.orders ?? 0}</div>
        </div>
        <div className="stat">
          ...
          {cells.map((c) => (
            <button
              ...
              style={{
                ...
                textAlign: "right",
                cursor: "pointer",
                borderColor: selected?.id === c.id ? "var(--signal)" : undefined,
              }}
              onClick={() => setSelected(c)}
            >
              <span>
                {c.title || `${c.trafficGb ?? "∞"}GB / ${c.months}م`}
              </span>
      ...
      <div className="panel">
        ...
        <div className="list">
          ...
          {!configs.length && <p className="muted">موردی نیست.</p>}
        </div>
      </div>
    </DashShell>
  );
