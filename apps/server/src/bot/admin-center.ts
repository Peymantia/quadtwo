...
async function showMiniPinPanel(ctx: Context, edit = true) {
  ...
  const msg = [
    ...
    `پین خودکار در /start و /update: ${auto ? "✅ روشن" : "⭕️ خاموش (پیش‌فرض)"}`,
    `کاربران ربات: ${total}`,
    "",
    "متن فعلی:",
    preview,
    "",
    "به‌صورت پیش‌فرض خاموش است؛ فقط اگر از همین‌جا فعال کنید، روی /start پین می‌شود.",
...
async function showPartnerRequestDetail(ctx: Context, requestId: string) {
  ...
  const text = [
    ...
    "",
    `برند: ${r.fullName}`,
    r.note?.trim() ? `نام: ${r.note.trim()}` : "نام: —",
    `📱 ${r.phone ?? "—"}`,
    `@${r.user.username ?? "—"}`,
