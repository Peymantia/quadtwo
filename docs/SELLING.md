...
- در تلگرام دکمه **تغییر نقش دمو** کنار خرید → ادمین / همکار / عمده / کاربر (هر دو سبز)
- در داشبورد وب نوار زرد بالای صفحه همان سوییچ را دارد
- نقش فقط در سشن دمو عوض می‌شود؛ نقش واقعی دیتابیس ثابت می‌ماند
- قیمت‌ها فقط بر اساس نقش دموی فعلی (مثل تجربه واقعی) — ادمین = رایگان
- با ورود هر کاربر، چند اکانت نمونه محلی در «سرویس‌های من» ساخته می‌شود (۳۰/۵۰ گیگ، نامحدود، تست یک‌روزه و …)
- لایسنس در دمو چک نمی‌شود
- **هیچ نوشتنی روی پنل 3x-ui انجام نمی‌شود** (add/update/delete کلاینت قفل است)
- خرید / تست / تمدید فقط ردیف محلی در دیتابیس می‌سازد با لینک `demo.invalid` — اتصال واقعی برقرار نمی‌شود
- این نصب را از فروش مشتری جدا نگه دارید (VPS / دامنه جدا؛ در صورت امکان `DATABASE_URL` جدا تا دادهٔ دمو با پروداکشن قاطی نشود)

### امنیت پنل در دمو

نگران پر شدن پنل اصلی نباشید: در `DEMO_MODE` کلاینت واقعی روی سنایی ساخته نمی‌شود. دیتابیس فیک جدا لازم نیست مگر بخواهید دادهٔ نمایشی از دیتابیس اصلی جدا باشد — کافی است `.env` دمو `DATABASE_URL` دیگری داشته باشد.
...
# پنل — در DEMO_MODE نوشتن روی پنل قفل است؛ خواندن اختیاری است
# XUI_BASE_URL=...
# XUI_API_TOKEN=...
# پیشنهاد: DATABASE_URL جدا برای نصب دمو تا داده با پروداکشن قاطی نشود
